...
exports.getAllCotizaciones = async (req, res) => {
    try {
        ...
        const estado = (req.query.estado || "Todos").trim();
        const offset = (page - 1) * limit;

        // ── WHERE principal para Cotizacion ──
        const whereCotizacion = {
            EstadoID: { [Op.ne]: 5 }  // excluir "Procesada/Convertida"
        };

        // ── Búsqueda: por ID numérico O por nombre/documento del usuario ──
        if (search) {
            if (!isNaN(search)) {
                // Si es número, buscar por CotizacionID exacto O por nombre/doc del usuario
                whereCotizacion[Op.or] = [
                    { CotizacionID: parseInt(search) },
                    { '$usuario.Nombre$':      { [Op.like]: `%${search}%` } },
                    { '$usuario.DocumentoID$': { [Op.like]: `%${search}%` } }
                ];
            } else {
                // Si es texto, buscar solo por nombre o documento
                whereCotizacion[Op.or] = [
                    { '$usuario.Nombre$':      { [Op.like]: `%${search}%` } },
                    { '$usuario.DocumentoID$': { [Op.like]: `%${search}%` } }
                ];
        ...
        const whereEstado = {};
        if (estado !== "Todos") {
            whereEstado.Nombre = estado;
        }

        // ── 1. COUNT separado (sin hasMany para evitar multiplicación) ──
        const totalCount = await Cotizacion.count({
            ...
            include: [
                {
                    ...
                    as: 'usuario',
                    attributes: [],
                    required: !!search   // INNER JOIN solo cuando hay búsqueda
                },
                {
                    model: Estado,
                    as: 'estado',
                    ...
                    required: estado !== "Todos"
                }
            ],
            subQuery: false  // necesario para que $usuario.X$ funcione en WHERE
        });

        // ── 2. QUERY de datos (sin subQuery:false para que LIMIT funcione bien con hasMany) ──
                ...
                },
                {
                    ...
                    include: [
                        { model: Producto, as: 'producto' },
                        {
                            ...
                            include: [{ model: Tecnica, as: 'tecnica' }, { model: Parte, as: 'parte' }]
                        },
                        { model: CotizacionTalla, as: 'tallas', include: [{ model: Talla, as: 'talla' }] },
                        { model: CotizacionColor, as: 'colores', include: [{ model: Color, as: 'color' }] },
                        { model: CotizacionInsumo, as: 'insumos', include: [{ model: Insumo, as: 'insumo' }] }
