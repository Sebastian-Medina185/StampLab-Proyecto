// src/pages/formularios_dash/ClientesForm.jsx
...
const ClientesForm = ({ onClose, onSave, cliente = null }) => {
    ...
    useEffect(() => {
        if (cliente) {
            setFormData({
                DocumentoID: cliente.DocumentoID ? cliente.DocumentoID.toString() : "",
    ...
    const handleChange = (e) => {
        ...
        switch (name) {
            ...
            case "Contraseña": error = validarContraseña(value, !!cliente); break;
            default: break;
        }
    ...
    const validateForm = () => {
        const newErrors = {
            ...
            Contraseña: validarContraseña(formData.Contraseña, !!cliente),
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (cliente) { if (formData.Contraseña?.trim()) dataToSend.Contraseña = formData.Contraseña.trim(); }
            else dataToSend.Contraseña = String(formData.Contraseña).trim();

            const result = cliente
                ? await updateUsuario(cliente.DocumentoID, dataToSend)
                : await createUsuario(dataToSend);

            if (result.estado) {
                Swal.mixin({ toast: true, position: "top-end", showConfirmButton: false, timer: 3000, timerProgressBar: true })
                    .fire({ icon: "success", title: cliente ? "Cliente actualizado correctamente" : "Cliente creado correctamente" });
                onSave();
            } else throw new Error(result.mensaje || "Error al guardar cliente");
        } catch (error) {
            console.error("Error al guardar cliente:", error);
            Swal.fire({ icon: "error", title: "Error", text: error.response?.data?.mensaje || error.message || "Error al guardar el cliente", confirmButtonColor: C.danger });
        } finally {
            setLoading(false);
        }
    };
                ...
                <form className="row g-3" onSubmit={handleSubmit}>
                    ...
                    <div className="col-12">
                        <label className="form-label fw-bold" style={{ color: C.navy }}>
                            Contraseña
                            {cliente && <small style={{ color: C.muted }}> (opcional)</small>}
                        </label>
                        <input
                            type="password"
                    ...
                    <div className="col-12 d-flex justify-content-center gap-2">
                        ...
                        >
                            {loading ? (
                                <><span className="spinner-border spinner-border-sm me-2" role="status"></span>{cliente ? "Actualizando..." : "Guardando..."}</>
                            ) : (
