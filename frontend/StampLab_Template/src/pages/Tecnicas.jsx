            ...
            <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 560, boxShadow: "0 8px 40px rgba(0,0,0,0.18)", overflow: "hidden", fontFamily: "'Outfit',sans-serif" }}>
                ...
                <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ background: C.bg, borderRadius: 10, padding: "12px 14px" }}>
                        <p style={{ margin: 0, fontSize: 11, color: C.muted, fontWeight: 600 }}>Nombre de la Técnica</p>
                ...
                <div style={{ padding: "0 24px 20px", display: "flex", justifyContent: "flex-end", gap: 10 }}>
                    <button onClick={() => { onClose(); onEdit(tecnica); }} style={{ background: C.accentSoft, color: C.accent, border: `1.5px solid ${C.accentBorder}`, borderRadius: 10, padding: "8px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Outfit',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
...
const Tecnicas = () => {
    ...
    const loadTecnicas = async () => {
        ...
        } catch (err) { setError("Error al cargar las técnicas: " + err.message); }
        finally { setLoading(false); }
    };

    const filtered = tecnicas.filter(t => t.Nombre.toLowerCase().includes(search.toLowerCase()));
        ...
        try {
            ...
            if (response.estado) {
                ...
                Swal.fire({
                    ...
                    text: response.message || "Error al eliminar la técnica.",
                    confirmButtonColor: C.danger,
                });
            }
        } catch (err) {
            const mensaje =
                ...
                err.message ||
                "No se pudo eliminar la técnica.";
    ...
    const handleCambiarEstado = async (tecnica) => {
        ...
        if (!result.isConfirmed) return;
        try {
            const response = await updateTecnica(tecnica.TecnicaID, { Nombre: tecnica.Nombre, Descripcion: tecnica.Descripcion, imagenTecnica: tecnica.imagenTecnica, Estado: nuevoEstado });
            if (response.estado) {
                await loadTecnicas();
    ...
    if (showForm) return <TecnicasForm onClose={handleCloseForm} onSave={handleSave} tecnicaEdit={tecnicaEdit} tecnicasExistentes={tecnicas} />;
    ...
    return (
      ...
      `}</style>
            ...
            <div style={{ minHeight: "100vh", background: C.bg, padding: "28px 32px", fontFamily: "'Outfit',sans-serif" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
                    ...
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                        <input type="text" placeholder="Buscar técnica..." value={search} onChange={e => setSearch(e.target.value)}
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filtered.map((t, idx) => (
                                    <tr key={t.TecnicaID} className="tec-row" style={{ background: idx % 2 === 0 ? "#fff" : C.accentSoft, borderBottom: `1px solid ${C.border}`, transition: "background 0.15s" }}>
                                        <td style={{ padding: "11px 14px", fontWeight: 600, color: C.navy }}>{t.Nombre}</td>
                                        <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>
                                            {t.Descripcion && t.Descripcion.length > 60 ? t.Descripcion.substring(0, 60) + "..." : t.Descripcion}
                                        </td>
            ...
            <DetailModal show={showDetailModal} onClose={() => setShowDetailModal(false)} tecnica={selectedTecnica} onEdit={t => { setTecnicaEdit(t); setShowForm(true); }} />
        </>
    );
};
