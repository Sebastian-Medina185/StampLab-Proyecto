...
import ClientesForm from "./formularios_dash/ClientesForm";
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderTop: `1px solid ${C.border}`, background: "#fff", flexWrap: "wrap", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 13, color: C.muted, fontFamily: "'Outfit',sans-serif" }}>
                    Mostrando <strong style={{ color: C.navy }}>{desde}–{hasta}</strong> de <strong style={{ color: C.navy }}>{totalItems}</strong> clientes
                </span>
...
/* ── Modal detalle ── */
const DetailModal = ({ show, onClose, cliente, onEdit }) => {
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 520, boxShadow: "0 8px 40px rgba(0,0,0,0.18)", overflow: "hidden", fontFamily: "'Outfit',sans-serif" }}>
                <div style={{ background: C.navyGrad, padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                        <p style={{ margin: 0, fontWeight: 800, fontSize: 16, color: "#fff" }}>Detalles del Cliente</p>
                        <p style={{ margin: 0, color: "rgba(255,255,255,0.7)", fontSize: 12 }}>Documento: {cliente.DocumentoID}</p>
                    </div>
                ...
                <div style={{ padding: "0 24px 20px", display: "flex", justifyContent: "flex-end", gap: 10 }}>
                    <button onClick={() => { onClose(); onEdit(cliente); }}
...
const Clientes = () => {
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            setPaginaActual(1);
            loadClientes(1, itemsPorPagina, search);
        }, 350);
        return () => clearTimeout(timeout);
    }, [search]);

    /* ── Función principal de carga ── */
    const loadClientes = async (page, limit, busqueda) => {
        try {
            setLoading(true);
            setError(null);
            const data = await getUsuarios({ page, limit, search: busqueda, rolID: 2 });
            // El backend ahora devuelve { datos, total, pagina, totalPaginas, limit }
    ...
    const handleAgregar     = () => { setSelectedCliente(null); setShowForm(true); };
    const handleEditar      = (c) => { setSelectedCliente(c); setShowForm(true); };
    const handleCloseForm   = () => { setShowForm(false); setSelectedCliente(null); };
    const handleSaveSuccess = async () => { await loadClientes(paginaActual, itemsPorPagina, search); handleCloseForm(); };
    const handleVer         = (c) => { setSelectedCliente(c); setShowDetailModal(true); };
    ...
    const handleEliminar = async (documentoID) => {
        try {
            ...
            if (cotizacionesActivas.length > 0) {
                await Swal.fire({ icon: "warning", title: "No se puede eliminar", html: `Este cliente tiene <strong>${cotizacionesActivas.length} cotización(es) activa(s)</strong>.`, confirmButtonText: "Entendido", confirmButtonColor: C.warning });
            ...
            if (ventasActivas.length > 0) {
                await Swal.fire({ icon: "warning", title: "No se puede eliminar", html: `Este cliente tiene <strong>${ventasActivas.length} venta(s) registrada(s)</strong>.`, confirmButtonText: "Entendido", confirmButtonColor: C.warning });
                setLoading(false); return;
            }

            const result = await Swal.fire({
                title: "¿Está seguro?",
                html: `<p>Está a punto de eliminar el cliente con documento <strong>${documentoID}</strong>.</p><p style="color:${C.danger};font-weight:600">Esta acción no se puede deshacer.</p>`,
            ...
            if (result.isConfirmed) {
                ...
                if (response.estado) {
                    await loadClientes(paginaActual, itemsPorPagina, search);
                    Toast.fire({ icon: "success", title: "Cliente eliminado correctamente" });
                } else {
                    throw new Error(response.mensaje || "Error al eliminar el cliente");
                }
            }
        } catch (err) {
            const errorMsg = err.response?.data?.mensaje || err.message || "Error al eliminar el cliente";
    ...
    /* ── Paginado (todo en backend, estos son solo para la UI) ── */
    ...
    const clientesPagina = clientes; // ya viene paginado del backend

    const handleCambiarPagina = (nuevaPagina) => {
        if (nuevaPagina >= 1 && nuevaPagina <= totalPaginas) {
            setPaginaActual(nuevaPagina);
            loadClientes(nuevaPagina, itemsPorPagina, search);
        }
    };
    ...
    const handleCambiarItemsPorPagina = (n) => {
        setItemsPorPagina(n);
        setPaginaActual(1);
        loadClientes(1, n, search);
    };

    if (showForm) return <ClientesForm onClose={handleCloseForm} onSave={handleSaveSuccess} cliente={selectedCliente} />;
        ...
        <>
            <style>{`
                ...
                * { box-sizing: border-box; }
                ...
                .cli-row:hover { background: ${C.accentSoft} !important; }
            ...
            <div style={{ minHeight: "100%", background: C.bg, padding: "28px 32px", fontFamily: "'Outfit',sans-serif" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
                    <div>
                        <h4 style={{ margin: 0, fontWeight: 800, fontSize: 22, color: C.navy, letterSpacing: "-0.02em" }}>Gestión de Clientes</h4>
                    ...
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                        ...
                        <button onClick={handleAgregar} disabled={loading}
                            ...
                            <FaPlusCircle size={15} /> Agregar Cliente
                        </button>
                    </div>
                </div>
                ...
                {error && (
                    <div style={{ background: "#fef2f2", border: `1.5px solid #fecaca`, borderRadius: 12, padding: "12px 18px", marginBottom: 20, color: C.danger, fontWeight: 600, fontSize: 13, display: "flex", alignItems: "center", gap: 12 }}>
                        {error}
                        <button onClick={() => loadClientes(1, itemsPorPagina, search)} style={{ background: C.danger, color: "#fff", border: "none", borderRadius: 8, padding: "5px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Outfit',sans-serif" }}>Reintentar</button>
                    </div>
                )}
                ...
                <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 16px rgba(0,0,0,0.07), 0 0 0 1px #f1f5f9", overflow: "hidden", animation: "fadeIn 0.3s ease" }}>
                    <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>
                            <thead>
                                <tr>
                                    {["Documento", "Nombre", "Correo", "Dirección", "Teléfono", "Acciones"].map((h, i) => (
                                        <th key={i} style={{ ...TH, textAlign: i === 5 ? "center" : "left", borderTopLeftRadius: i === 0 ? 18 : 0, borderTopRightRadius: i === 5 ? 18 : 0 }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {loading ? (
                                    <tr><td colSpan={6} style={{ textAlign: "center", padding: "48px 0" }}>
                                        ...
                                        <p style={{ color: C.muted, margin: 0, fontSize: 13 }}>Cargando clientes...</p>
                                    </td></tr>
                                ) : error ? (
                                    <tr><td colSpan={6} style={{ textAlign: "center", padding: "48px 0" }}>
                                        <p style={{ color: C.danger, fontWeight: 600, marginBottom: 12 }}>❌ Error al cargar datos</p>
                                        <button onClick={() => loadClientes(1, itemsPorPagina, search)} style={{ background: C.accentSoft, color: C.accent, border: `1.5px solid ${C.accentBorder}`, borderRadius: 9, padding: "7px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Outfit',sans-serif" }}>Reintentar</button>
                                    </td></tr>
                                ) : clientes.length === 0 ? (
                                    <tr><td colSpan={6} style={{ textAlign: "center", padding: "48px 0" }}>
                                        ...
                                        {!search && <button onClick={handleAgregar} style={{ background: C.navy, color: "#fff", border: "none", borderRadius: 10, padding: "9px 20px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Outfit',sans-serif" }}>Crear primer cliente</button>}
                                    </td></tr>
                                ) : (
                                    clientesPagina.map((c, idx) => (
                                        <tr key={c.DocumentoID} className="cli-row" style={{ background: idx % 2 === 0 ? "#fff" : C.accentSoft, borderBottom: `1px solid ${C.border}`, transition: "background 0.15s" }}>
                                            <td style={{ padding: "11px 14px" }}><Badge type="muted">{c.DocumentoID}</Badge></td>
                                            <td style={{ padding: "11px 14px", fontWeight: 600, color: C.navy }}>{c.Nombre}</td>
                                            <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>{c.Correo}</td>
                                            <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>
                                                {(c.Direccion || "").length > 40 ? (c.Direccion || "").substring(0, 40) + "..." : (c.Direccion || "")}
                                            </td>
                                            <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>{c.Telefono || ""}</td>
                                            <td style={{ padding: "11px 14px", textAlign: "center" }}>
                                                <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                                                    <button className="icon-btn" onClick={() => handleVer(c)} title="Ver detalles"
                                                    ...
                                                    <button className="icon-btn" onClick={() => handleEditar(c)} title="Editar"
                                                    ...
                                                    <button className="icon-btn" onClick={() => handleEliminar(c.DocumentoID)} title="Eliminar"
            ...
            <DetailModal
                ...
                onEdit={(c) => { setSelectedCliente(c); setShowForm(true); }}
            />
        </>
    );
};
